...
DESCRIPTION
  Browser Support Spec is a command-line tool, which runs in the NodeJS runtime environment. As
  such installing node10 or newer is required for this software to run. It runs queries on the
  CanIUseAPI to generate browser support lists containing the names of all the web browsers which
  supports all of the required web features specified in the query.
  ...
  search
    search is used to list all web technologies whose name matches with the argument "keyword".
    This command is useful for getting to know the different identifiers present in the CanIUse
    database.
  list
...
// case G
it.only("breaks lines in the help text that are over 100 characters long", () => {
